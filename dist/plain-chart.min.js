...
/*!
   ...
   * Copyright (c) 2020-present hai2007 走一步，再走一步。
   * Released under the MIT license
   */var getValue=function getValue(target,express,scope){if(arguments.length<3)scope={};var expressArray=analyseExpress(target,express,scope);var path=toPath(target,expressArray,scope);return calcValue(target,path,scope)};var setValue=function setValue(target,express,value,scope){if(arguments.length<3)scope={};var expressArray=analyseExpress(target,express,scope);var path=toPath(target,expressArray,scope);var _target=target;for(var i=0;i<path.length-1;i++){if(!(path[i]in _target))_target[path[i]]=isArray(_target)?[]:{};_target=_target[path[i]]}_target[path[path.length-1]]=value;return target};function merge(option,newOption){(function doMerge(express,source){for(var key in source){var newExpress=express+"['"+key+"']";if(isObject(source[key])&&!isArray(source[key])){if(!getValue(option,newExpress)){setValue(option,newExpress,{})}doMerge(newExpress,source[key])}else{setValue(option,newExpress,source[key])}}})("",newOption);return option}function chart(clunch,$ruler){var options=[];var chartManager={new:function _new(option){var order=options.length;options.push(option);setTimeout((function(){clunch.options.push(format(option,clunch,$ruler));clunch.flag+=1}),300);return{setOption:function setOption(newOption){options[order]=merge(options[order],newOption);clunch.options[order]=format(options[order],clunch,$ruler);clunch.flag+=1}}}};return chartManager}var bar=["number","json","$getLoopColors",function($number,$json,$getLoopColors){return{attrs:{x:$number(),y:$number(),width:$number(),height:$number(),data:$json()},link:function link(painter,attr){painter.config({fillStyle:$getLoopColors(1)[0]});var w1=attr.width/attr.data.values.length;var h1=-1/(attr.data.max-attr.data.min)*attr.height;for(var i=0;i<attr.data.values.length;i++){painter.fillRect(attr.x+w1*(i+.1),attr.y,w1*.8,(attr.data.values[i]-attr.data.min)*h1)}}}}];var line=["number","json","$getLoopColors",function($number,$json,$getLoopColors){return{attrs:{x:$number(),y:$number(),width:$number(),height:$number(),data:$json()},link:function link(painter,attr){painter.config({strokeStyle:$getLoopColors(1)[0]});var w1=attr.width/attr.data.values.length;var h1=-1/(attr.data.max-attr.data.min)*attr.height;painter.beginPath();for(var i=0;i<attr.data.values.length;i++){painter.lineTo(attr.x+w1*(i+.5),attr.y+(attr.data.values[i]-attr.data.min)*h1)}painter.stroke()}}}];var pie=["number","json","string","$getLoopColors",function($number,$json,$string,$getLoopColors){return{attrs:{cx:$number(),cy:$number(),radius:$number(),type:$string(),values:$json()},link:function link(painter,attr){var colors=$getLoopColors(attr.values.length);var allValue=0;var _iterator=_createForOfIteratorHelper(attr.values),_step;try{for(_iterator.s();!(_step=_iterator.n()).done;){var value=_step.value;allValue+=value}}catch(err){_iterator.e(err)}finally{_iterator.f()}var beginDeg=Math.PI*-.5;for(var index in attr.values){var deg=attr.values[index]/allValue*Math.PI*2;painter.config({fillStyle:colors[index]}).fillArc(attr.cx,attr.cy,(attr.type=="ring"?.5:0)*attr.radius,attr.radius,beginDeg,deg);beginDeg+=deg}}}}];var serieslist={"plain-chart-bar":bar,"plain-chart-line":line,"plain-chart-pie":pie};var server={};var options={render:image,debug:false,data:["$ruler",function($ruler){server={$ruler:$ruler};return{options:[],flag:0}}]};var PlainChart=function PlainChart(init){return new Promise((function(resolve){init(options,serieslist).then((function(clunch){resolve(chart(clunch,server.$ruler))}))}))};PlainChart.init=function(Clunch,el){Clunch.series(serieslist);options.el=el;return chart(new Clunch(options),server.$ruler)};if((typeof module==="undefined"?"undefined":_typeof(module))==="object"&&_typeof(module.exports)==="object"){module.exports=PlainChart}else{window.PlainChart=PlainChart}})();